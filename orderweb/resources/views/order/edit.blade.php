                ...
                <div class="col-lg-4 mb-4">
                    ...
                    <select name="causal_id" id="causal_id" class="form-control">
                        ...
                        @foreach ($causals as $causal)
                            <option value="{{ $causal['id'] }}" @if($causal['id'] == $order['causal_id']) selected @endif>{{ $causal['description'] }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-lg-4 mb-4">
                    ...
                    <select name="observation_id" id="observation_id" class="form-control">
                        ...
                        @foreach ($observations as $obsevation)
                            <option value="{{ $obsevation['id'] }}" @if($obsevation['id'] == $order['observation_id']) selected @endif>{{ $obsevation['description'] }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
